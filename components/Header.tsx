...
const Header: React.FC<HeaderProps> = ({ config }) => {
  ...
  return (
    <>
          ...
          <Link to="/" className="flex items-center max-w-[45%] xs:max-w-[55%] sm:max-w-none" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
            <div className="flex items-center">
              {logo ? (
                ...
                <div className="bg-green-600 p-2 rounded-lg h-8 w-8 sm:h-10 sm:w-10 flex items-center justify-center mr-2 sm:mr-3 shrink-0 shadow-lg shadow-green-900/10">
              ...
              <span className="font-black text-[10px] xs:text-[11px] sm:text-lg md:text-xl tracking-tighter sm:tracking-tight uppercase flex flex-wrap items-center leading-tight">
                ...
                {businessNamePart4 && <span className="ml-1 opacity-60" style={{ color: businessNameColor4 || '#6b7280' }}>{businessNamePart4}</span>}
              </span>
            </div>
          </Link>
          ...
          <div className="flex items-center space-x-1 sm:space-x-2 md:space-x-3">
            ...
            {showPhoneNumber && (
              <a 
                ...
                className="hidden lg:flex items-center space-x-2 bg-green-50 text-green-700 px-5 py-2.5 rounded-full font-black transition-all border border-green-100 hover:bg-green-100"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                </svg>
                <span className="text-sm tracking-widest">{phoneNumber}</span>
              </a>
            )}
            ...
            <div className="relative" ref={menuRef}>
              ...
              <div className={`absolute right-0 mt-4 w-72 sm:w-80 bg-white rounded-2xl shadow-2xl border border-gray-100 py-4 transition-all duration-300 origin-top-right transform ${isMenuOpen ? 'opacity-100 scale-100 pointer-events-auto' : 'opacity-0 scale-95 pointer-events-none'}`}>
                <div className="px-6 py-2 border-b border-gray-50 mb-2 flex justify-between items-center">
                  <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Master Navigation</span>
                </div>
                <div className="space-y-1">
                  {navItems.map((item) => (
                    <button
                      ...
                      className={`w-full text-left flex items-center px-6 py-4 text-base font-black uppercase tracking-tighter transition-all ${
                ...
                <div className="mt-4 px-4 pt-4 border-t border-gray-50">
                  {showPhoneNumber && (
                    <div className="bg-gray-50 rounded-2xl p-4">
                      <p className="text-[9px] font-black text-gray-400 uppercase mb-3 tracking-[0.2em] text-center">Instant Dispatch</p>
                      <a 
                        href={`tel:${phoneNumber.replace(/\D/g,'')}`}
                        className="flex items-center justify-center w-full bg-green-600 text-white py-4 rounded-xl font-black text-sm hover:bg-green-700 transition-all shadow-xl shadow-green-900/20 uppercase tracking-widest"
                      >
                        CALL {phoneNumber}
                      </a>
                    </div>
      ...
      {/* Map Modal */}
      {isMapModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-6 md:p-10 bg-black/85 backdrop-blur-sm animate-fade-in">
          <div className="relative w-full max-w-5xl bg-white rounded-[1.5rem] sm:rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col h-[85vh] sm:h-[80vh]">
            <div className="p-5 sm:p-6 flex justify-between items-center border-b border-gray-100">
              <div className="max-w-[80%]">
                <h3 className="text-lg sm:text-2xl font-black text-gray-900 uppercase tracking-tighter leading-tight">Milwaukee HQ Yard</h3>
                {showAddress && <p className="text-[9px] sm:text-xs text-gray-400 font-bold uppercase tracking-widest mt-1 line-clamp-1">{address}</p>}
              </div>
              <button 
                onClick={() => setIsMapModalOpen(false)}
                className="p-3 bg-gray-100 hover:bg-red-50 text-gray-500 hover:text-red-600 rounded-2xl transition-all"
              >
            ...
            <div className="p-4 sm:p-6 bg-gray-50 flex flex-col sm:flex-row gap-4 items-center justify-between">
              <div className="flex items-center space-x-3 text-left">
                 <div className="bg-green-600 p-2 rounded-lg text-white shrink-0 shadow-lg shadow-green-900/10">
                   <svg className="w-4 h-4 sm:w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                 </div>
                 <div className="text-[10px] sm:text-xs font-black text-gray-600 uppercase tracking-widest leading-relaxed">
                   Mon-Fri: 8am-6pm<br className="sm:hidden" /><span className="hidden sm:inline"> | </span>Sat: 8am-3pm
                 </div>
              </div>
              {showDirectionsButton && (
                <a 
                  ...
                  className={`w-full sm:w-auto text-white ${btnSizeClasses} ${btnShapeClasses} font-black uppercase tracking-widest shadow-xl shadow-green-900/20 hover:brightness-110 transition-all text-center text-xs py-4 sm:py-5`}
                >
                  {directionsButtonText}
                </a>
              )}
